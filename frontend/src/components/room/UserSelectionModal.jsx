...
import { FiUsers, FiLayers, FiCheck } from 'react-icons/fi';
...
const UserSelectionModal = ({ isOpen, onClose, user, onConfirm }) => {
  ...
  useEffect(() => {
    if (isOpen && user) {
      console.log('Modal opened with user:', user.name);
  ...
  useEffect(() => {
    console.log('UserSelectionModal props:', { isOpen, hasUser: !!user, showConfetti });
  }, [isOpen, user, showConfetti]);

  if (!user) {
    console.log('UserSelectionModal: No user data, returning null');
    return null;
  }
      ...
      >
        <div className="p-8 sm:p-10">
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            {user.group && (
              <span className="inline-block bg-primary-100 dark:bg-primary-900 text-primary-800 dark:text-primary-200 text-lg px-4 py-2 rounded-full">
            ...
            {user.level && (
              <span className="inline-block bg-secondary-100 dark:bg-secondary-900 text-secondary-800 dark:text-secondary-200 text-lg px-4 py-2 rounded-full">
            ...
            {user.mobile && (
              <span className="inline-block bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-lg px-4 py-2 rounded-full">
                {user.mobile}
              </span>
            )}
          </div>
